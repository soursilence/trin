...
<?php endif;
      ...
      foreach($this->rows as $row):
        $canView = isset($allowed_categories[$row->cid]); ?>
...
<?php   if($this->_config->get('jg_showminithumbs')):
          if($row->thumbnail && isset($row->id)): ?>
            <?php echo JHTML::_('joomgallery.minithumbcat', $row, 'jg_up_eminithumb', $canView, true); ?>
<?php     else: ?>
...
<?php   if($this->_user->authorise('core.edit', _JOOM_OPTION.'.category.'.$row->cid) || ($this->_user->authorise('core.edit.own', _JOOM_OPTION.'.category.'.$row->cid) && $this->_user->get('id') == $row->owner)): ?>
...
<?php     if(!$row->children && !$row->images): ?>
          <a href="javascript:if (confirm('<?php echo JText::_('COM_JOOMGALLERY_COMMON_ALERT_SURE_DELETE_SELECTED_ITEM',true); ?>')){ location.href='<?php echo JRoute::_('index.php?task=category.delete&catid='.$row->cid.$this->slimitstart, false); ?>';}">
...
<?php   if($this->_user->authorise('core.edit.state', _JOOM_OPTION.'.category.'.$row->cid)): ?>
          <a href="<?php echo JRoute::_('index.php?task=category.publish&catid='.$row->cid.$this->slimitstart); ?>"<?php echo JHTML::_('joomgallery.tip', $p_text, $p_title, true, false); ?>>
