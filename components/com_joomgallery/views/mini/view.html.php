...
// $Id: view.html.php 3978 2012-11-07 08:09:16Z chraneco $
/****************************************************************************************\
**   JoomGallery 2                                                                      **
  ...
  function display($tpl = null)
  {
    ...
    if($extended > 0)
    {
      ...
      if($params->get('upload_enabled'))
      {
        ...
        {
          $categories = $model->getUploadCategories($catids);
          if(!count($categories))
          {
          ...
          {
            $lists['upload_categories'] = JHtml::_('select.genericlist', $categories, 'catid', null, 'cid', 'path');
          }
        }
        else
        {
      ...
      if($params->get('create_category'))
      {
        ...
        {
          $categories = $model->getParentCategories($catids);
          if(!count($categories))
          {
          ...
          {
            $lists['parent_categories'] = JHtml::_('select.genericlist', $categories, 'parent_id', null, 'cid', 'path');
          }
        }
        else
        {
